      ...
      <a-form-model :model="form">
        ...
        <a-form-model-item label="所在仓库">
          ...
          <a-select v-model="form.department"  placeholder="请选择员工所在仓库"> 
            <a-select-option :value="item.name" :key="item.id" v-for="item in warehouseList" >{{ item.name }}</a-select-option>
  ...
  methods: {
    ...
    findAllWarehouse() {
      FindAllWarehouse().then((res) => {
        ...
        console.log('2执行中findAllWarehouse:'+this.warehouseList[0].name)
      })
    },

    loadTableData() {
      this.loading = true
      console.log('开始findAllWarehouse'+this.warehouseList)
      this.findAllWarehouse()
      console.log('结束findAllWarehouse'+this.warehouseList[0])
